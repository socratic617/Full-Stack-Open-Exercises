...
const App = () => {
  ...
  // State to keep track of the index of the currently selected anecdote
  const [selected, setSelected] = useState(0);

  // State to keep track of the number of votes each anecdote has received
  const [votes, setVotes] = useState(new Array(anecdotes.length).fill(0));

  // Function to handle clicking on the "Next Anecdote" button
  const handleNextAnecdote = () => {
    // Generate a random index to select a random anecdote from the array
    const randomIndex = Math.floor(Math.random() * anecdotes.length);
    // Update the selected anecdote index in the state
    setSelected(randomIndex);
  };

  // Function to handle clicking on the "Vote" button
  const handleVote = () => {
    // Create a copy of the votes array
    const newVotes = [...votes];
    // Increment the vote count for the currently selected anecdote
    newVotes[selected] += 1;
    // Update the votes state with the new vote count
    setVotes(newVotes);
  };
  ...
  return (
    <div>
      ...
      <p>{anecdotes[selected]}</p>
      {/* Display the number of votes the currently selected anecdote has */}
      <p>Has {votes[selected]} votes</p>
      {/* Button to vote for the currently displayed anecdote */}
      <button onClick={handleVote}>Vote</button>
      {/* Button to select a new random anecdote */}
      <button onClick={handleNextAnecdote}>Next Anecdote</button>

      <h2>Top Anecdote</h2>
      {/* Display the top-voted anecdote */}
      <p>{anecdotes[votes.indexOf(Math.max(...votes))]}</p>
      {/* Display the number of votes the top-voted anecdote has */}
